		...
		$(result).each(function(i,result){
			...
			for (item in result) { 
				...
				if (typeof result[item][subItem].title !== "undefined") {
				       $('#newsData tr:last').after("<tr><td>"+result[item][subItem].title+"</td><td>"+result[item][subItem].description+"</td><td>"+result[item][subItem].publishedAt+"</td><td>"+result[item][subItem].author+"</td><td><img src='"+result[item][subItem].urlToImage+"' alt='image' width='100px;' height='100px;' /></td><td><form action='detail.php' method='post'><input type='hidden' name='title' value="+result[item][subItem].title+"/><input type='submit' value='View Detail' class='btn btn-warning'/></form></td></tr>");
				}
					//$("div").append("<b>"+result[item][subItem].title+"</b>");
					
						console.log(result[item][subItem]);
